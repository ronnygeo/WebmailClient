	...
	function SelectFolder($folderName)
		...
		function FetchList($start, $end)
		{
			...
			try
			{
				...
				// Must get lines with * and cmdTag + OK or cmdTag BAD/NO
				$reply = socket_read($this->_socket, 5024 );
				$fd=0;
				$lines = explode( "\n",$reply);
				...
				foreach($lines as $line)
				{
					...
					if($line[0] =="*"&&$fd==0)
					{
						...
						$strRow .= "<tr id=\"".trim($temp). "\"" . " onclick=\"Get_Mail(this.id);\""; //"newmail.php?id="+temp
					...
					elseif((strpos($line, "From:") > -1)&&$fd==0){
						$temp =substr($line,5);	
					...
					elseif((strpos($line, "Subject:") > -1)&&$fd==0){
						$temp =substr($line,8);	
						$strSubject ="<td>$temp</td> ";
					
					}
					elseif((strpos($line, "Date:") > -1)&&$fd==0){
						$temp =substr($line,5);	
						$strDate ="<td>$temp</td> ";
	...
	function delete($id){
		...
		function Logout(){
			...
			if($this->m_Authenticated){
				...
				try
				{
					socket_write($this->_socket,"L1 LOGOUT\n");
				...
				{
					$this->mail_output("Error Logout".$e1);
				}
